...
// --- Type Definitions moved to top scope ---
enum CallStatus {
  INACTIVE = "INACTIVE",
  CONNECTING = "CONNECTING",
  ACTIVE = "ACTIVE",
...
const Agent = ({
  ...
  }, [currentInterviewId, userId, feedbackId, router]);
  ...
  useEffect(() => {
    ...
    // --- FIXED: Removed duplicate Enum/Interface definitions from here ---
  ...
  const handleCall = async () => {
    ...
    // Ensure we have an interview ID if this is a new generation
    if (type === "generate" && !idToCall) {
      try {
        const interviewParams = {
          userId: userId!,
